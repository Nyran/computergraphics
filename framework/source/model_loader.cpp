...
std::vector<glm::fvec3> generate_tangents(tinyobj::mesh_t const& model) {
    ...
    for (unsigned i = 0; i < model.indices.size() / 3; i++) {
		...
		try{
			glm::fvec3 deltaP_1 = positions[indices[1]] - positions[indices[0]];
			glm::fvec3 deltaP_2 = positions[indices[2]] - positions[indices[0]];

			glm::fvec2 deltaUV_1 = texcoords[indices[1]] - texcoords[indices[0]];
			glm::fvec2 deltaUV_2 = texcoords[indices[2]] - texcoords[indices[0]];
    ...
    // implement orthogonalization and normalization here
		...
		std::cout << i << std::endl;*/
		try {
			glm::fvec3 tmp_tangent = tangents[i] - normals[i] * glm::dot(normals[i], tangents[i]);

			tangents[i] = glm::normalize(tmp_tangent);
		}
		catch (std::exception e) {
			std::cerr << e.what() << std::endl;
		}
